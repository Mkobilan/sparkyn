...
export default function FacebookDashboard() {
  ...
  const handleGenerate = async (accountId: string, publishNow: boolean = false, isVideo: boolean = false) => {
    ...
    try {
      ...
      // ── WATERFALL STEP 2: GENERATE MEDIA (IMAGE/VIDEO) ──
      setProgressMsg(isVideo ? "Step 2/3: Compiling AI Video (this takes 30s)..." : "Step 2/3: Generating AI Image...")
      ...
      if (publishNow) {
        setProgressMsg("Step 3/3: Posting live to Facebook...")
        ...
        alert(`✅ Published successfully to your Facebook page!`)
      } else {
      ...
      setErrorModal(isTimeout 
        ? 'The request timed out at a specific step. This usually means the AI model is busy—please try again.'
    ...
    <div className="text-white relative">
      ...
      {progressMsg && (
        <div className="fixed inset-0 bg-[#000000]/80 z-[99999] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#0f0f0f] border border-[#1877F2]/30 p-10 rounded-[2.5rem] max-w-md w-full shadow-[0_0_80px_rgba(24,119,242,0.2)] text-center space-y-6">
            <div className="flex justify-center">
              <RefreshCw className="w-16 h-16 text-[#1877F2] animate-spin" />
            </div>
            <div className="space-y-2">
              <h3 className="text-2xl font-black text-white tracking-tight">Processing Waterfall</h3>
              <p className="text-[#1877F2] font-bold animate-pulse">{progressMsg}</p>
            </div>
            <p className="text-muted-foreground text-xs px-4 leading-relaxed">
              We are splitting your request into multiple stages to prevent timeouts. 
